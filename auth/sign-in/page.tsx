...
const SignIn = () => {
  ...
  const handleSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      const response = await authAxiosInstance.post('/auth/login', values); // Sets JWT token as a httpOnly cookie
  ...
  return (
    <div className='relative rounded-lg w-full max-w-lg px-4 py-6'>
      ...
      <div className='relative my-6'>
        <hr />
        <span className='absolute right-1/2 -top-3 bg-white dark:bg-black translate-x-1/2 px-2 text-center text-grey-base text-sm'>
          or
        </span>
      </div>
      ...
      <div className='block md:hidden justify-center'>
        <p className='40 text-xs text-center mt-8'>
